...
fn MessageType(comptime Handler: type) type {
    // Each handler method is matched against the full LSP request/notification method sets at comptime
    // (isRequestMethod/isNotificationMethod -> StaticStringMap.has). With a rich handler surface the
    // cumulative comptime branch count exceeds Zig's default 1000, so raise it here.
